...
export default function Settings() {
  ...
  const auth = authUser as unknown as { name?: string; email?: string; avatarUrl?: string; metadata?: Record<string, unknown> } | undefined;
  const getProp = <T, K extends string>(obj: unknown, key: K): T | undefined => {
    if (obj && typeof obj === 'object' && key in obj) return (obj as Record<string, unknown>)[key] as T | undefined;
    return undefined;
  };
  const userCtx = userContextUser as Partial<UserModel> | undefined;

  const profile = {
    firstname: userCtx?.firstname ?? (auth?.name ? String(auth.name).split(' ')[0] : undefined) ?? mock.firstname,
    lastname: userCtx?.lastname ?? (auth?.name ? String(auth.name).split(' ').slice(1).join(' ') : undefined) ?? mock.lastname,
    email: userCtx?.email ?? auth?.email ?? mock.email,
  // prefer explicit top-level phoneNumber from user/auth responses
  phoneNumber: userCtx?.phoneNumber ?? getProp<string, 'phoneNumber'>(auth, 'phoneNumber') ?? auth?.metadata?.phoneNumber ?? auth?.metadata?.phone ?? undefined,
    roles: userCtx?.roles ?? (Array.isArray(auth?.metadata?.roles) ? auth!.metadata!.roles : auth?.metadata?.roles ? [String(auth!.metadata!.roles)] : mock.roles),
    haloId: userCtx?.haloId ?? auth?.metadata?.haloId ?? mock.haloId,
    tenantId: userCtx?.tenantId ?? auth?.metadata?.tenantId ?? mock.tenantId,
    isEmailVerified: userCtx?.isEmailVerified ?? auth?.metadata?.isEmailVerified ?? mock.isEmailVerified,
    isPhoneVerified: userCtx?.isPhoneVerified ?? auth?.metadata?.isPhoneVerified ?? mock.isPhoneVerified,
  ...
  createdAt: userCtx?.createdAt ?? getProp<string, 'createdAt'>(auth, 'createdAt') ?? undefined,
  ...
  const memberSince = profile?.createdAt ? new Date(String(profile.createdAt)).toLocaleDateString() : null;
  ...
  if (typeof setAuthUser === 'function') setAuthUser(merged as unknown as UserModel);
    ...
    return (
      <div className="space-y-4">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-2">Halo ID</label>
            <input
              type="text"
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div>
            ...
            <div className="flex items-center">
              <span className={`inline-block w-3 h-3 mr-2 rounded-full ${(user?.isEmailVerified ?? profile?.isEmailVerified) ? 'bg-green-500' : 'bg-red-500'}`} />
              <span className="text-sm">{(user?.isEmailVerified ?? profile?.isEmailVerified) ? 'Yes' : 'No'}</span>
            </div>
          </div>
          ...
          <div>
            ...
            <div className="flex items-center">
              <span className={`inline-block w-3 h-3 mr-2 rounded-full ${(user?.isPhoneVerified ?? profile?.isPhoneVerified) ? 'bg-green-500' : 'bg-red-500'}`} />
              <span className="text-sm">{(user?.isPhoneVerified ?? profile?.isPhoneVerified) ? 'Yes' : 'No'}</span>
            </div>
          </div>
        </div>
        ...
        <div>
          ...
          <div className="inline-flex items-center px-3 py-1 rounded-full text-sm font-medium" style={{ background: (user?.isSuper ?? profile?.isSuper) ? '#ecfdf5' : '#fff1f2', color: (user?.isSuper ?? profile?.isSuper) ? '#065f46' : '#9f1239' }}>
    ...
    <div className="space-y-6">
      ...
      <div className="space-y-4">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-2">Halo ID</label>
            <input type="text" defaultValue={user?.haloId || mockUser[0]?.haloId} readOnly className="w-full px-3 py-2 border border-gray-300 rounded-lg bg-gray-100" />
          </div>
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-2">Tenant ID</label>
            <input type="text" defaultValue={user?.tenantId || mockUser[0]?.tenantId} readOnly className="w-full px-3 py-2 border border-gray-300 rounded-lg bg-gray-100" />
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-2">Email Verified</label>
            <div className="flex items-center">
              <span className={`inline-block w-3 h-3 mr-2 rounded-full ${user?.isEmailVerified ? 'bg-green-500' : 'bg-red-500'}`} />
          ...
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-2">Phone Verified</label>
            <div className="flex items-center">
              <span className={`inline-block w-3 h-3 mr-2 rounded-full ${user?.isPhoneVerified ? 'bg-green-500' : 'bg-red-500'}`} />
              <span className="text-sm">{user?.isPhoneVerified ? 'Yes' : 'No'}</span>
            </div>
          </div>
        </div>
        ...
        <div>
          <label className="block text-sm font-medium text-gray-700 mb-2">Admin</label>
          <div className="inline-flex items-center px-3 py-1 rounded-full text-sm font-medium" style={{ background: user?.isSuper ? '#ecfdf5' : '#fff1f2', color: user?.isSuper ? '#065f46' : '#9f1239' }}>
            {user?.isSuper ? 'Admin' : 'Not admin'}
          </div>
        </div>
      </div>      
    </div>
      ...
      <div>
        ...
        <div className="space-y-4">
          {([
            ...
            type NotificationItem = { key: NotificationKey; label: string; description: string };
            const it = item as unknown as NotificationItem;
            const k = it.key as NotificationKey;
            return (
              <div key={k} className="flex items-center justify-between">
                <div>
                  <h4 className="text-sm font-medium text-gray-900">{it.label}</h4>
                  <p className="text-sm text-gray-500">{it.description}</p>
                </div>
                <button
                  onClick={() => setNotifications(prev => ({ ...prev, [k]: !prev[k] }))}
